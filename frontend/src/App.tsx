...
const calculateSmartMatch = (projectSkills, studentSkills) => {
  if (!projectSkills || !projectSkills.length) return 80;
  const userStack = (studentSkills || []).map(s => s.toLowerCase().trim());
  const matched = projectSkills.filter(ps => 
    userStack.some(us => us.includes(ps.toLowerCase().trim()) || ps.toLowerCase().trim().includes(us))
  );
...
const CustomModal = ({ isOpen, onClose, title, subtitle, children, maxWidth = "max-w-xl" }) => {
...
export default function App() {
  ...
  const triggerToast = (message, variant = "check") => {
    const id = Date.now();
  ...
  useEffect(() => {
    // Hash Routing for Demo Mode
    const handleHash = () => {
      if (window.location.hash === '#demo') setCurrentModule('demo-pitch');
      else if (currentModule === 'demo-pitch') setCurrentModule('storefront');
    };
  ...
  const handleInstantHireContract = (project) => {
  ...
  const handleSubmitDeliverable = (e, contractId) => {
    e.preventDefault();
    const form = e.target;
  ...
  const handleApproveAndReleasePayment = (contractId) => {
        ...
        <div className="flex items-center justify-between h-16">
          ...
          {currentModule === 'demo-pitch' ? (
            <div className="hidden lg:flex items-center">
               <span className="bg-purple-100 text-purple-700 px-4 py-1.5 rounded-full text-xs font-bold border border-purple-200 flex items-center gap-2">
  ...
  const renderDemoPitchModule = () => (
    <div className="animate-in fade-in py-8">
      <div className="text-center mb-12">
        <Badge variant="purple" className="mb-4 text-sm px-4 py-1">
          <Sparkles size={16} className="mr-1" /> End-to-End Platform Overview
        </Badge>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-lg transition-shadow">
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
            <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center"><User size={24} /></div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-lg transition-shadow">
          ...
          <div>
            ...
            <div className="bg-slate-900 text-white p-5 rounded-xl shadow-inner">
              ...
              <button className="w-full bg-blue-600 hover:bg-blue-500 text-xs font-bold py-2.5 rounded-lg transition flex items-center justify-center gap-2"><CreditCard size={14}/> Checkout via Escrow</button>
            </div>
          </div>
        </div>

        {/* Pillar 3: Admin Side */}
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-lg transition-shadow relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-rose-50 rounded-bl-full -z-10"></div>
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
            <div className="w-12 h-12 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center"><Shield size={24} /></div>
      ...
      <div className="mt-10 text-center">
         <button onClick={() => { window.location.hash = ''; setCurrentModule('storefront'); }} className="px-6 py-2.5 bg-slate-100 text-slate-700 hover:bg-slate-200 text-sm font-bold rounded-full transition shadow-sm border border-slate-200">
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 antialiased flex flex-col selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* Floating Bottom Navigation Bar for Mobile (Hidden in Demo) */}
      {currentModule !== 'demo-pitch' && (
